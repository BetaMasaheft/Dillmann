...
<xsl:call-template name="sense2">
            ...
            <xsl:matching-substring>
                <xsl:variable name="string">
                    <xsl:choose>
                        ...
                        <xsl:when test="regex-group(1) = 'radd. Radd.'">radices</xsl:when>
                        <xsl:when test="regex-group(1) = 'vic. Vic.'">vicinus</xsl:when>
                        ...
                        <xsl:when test="regex-group(1) = 'coll.'">collatio, -is vel collectivum, -e</xsl:when>
                        ...
                        <xsl:when test="regex-group(1) = 'Form. Conf.'">Formula Confessionis</xsl:when>
                        ...
                        <xsl:when test="regex-group(1) = 'Vers. Lat.'">Versio Latina Jubilaeorum libri, exstat in libro: Monumenta sacra et profana e codicibus Bibl. Ambrosianae tom. I, fasc. I. ed Ceriani, Mediol. 1861. 4°.</xsl:when>
                        ...
                        <xsl:when test="regex-group(1) = 'concr.'">concretus, e (opp. abstracto)</xsl:when>
            ...
            <xsl:matching-substring>
                <xsl:variable name="string">
                    <xsl:choose>
                        ...
                        <xsl:when test="regex-group(1) = 'Infin. nom.'">infinitivus nominalis, nominascens</xsl:when>
                        ...
                        <xsl:when test="regex-group(1) = 'Infin. verb.'">infinitivus verbalis</xsl:when>
    ...
    <xsl:template name="case">
        ...
        <xsl:analyze-string regex="(@(.*?)@)" select="$text8">
            <xsl:matching-substring>
                <xsl:variable name="string">
                    <xsl:choose>
    ...
    <xsl:template name="gender">
        ...
        <xsl:analyze-string regex="(ˆ(.*?)ˆ)" select="$text9">
            <xsl:matching-substring>
                <xsl:variable name="string">
                    <xsl:choose>
    ...
    <xsl:template name="sup">
        ...
        <xsl:analyze-string regex="(ˆ!(.*?))" select="$text10">
                <xsl:matching-substring>
                    <sup>
                    <xsl:value-of select="regex-group(2)"/>
                </sup>
    ...
    <xsl:template name="note">
        ...
        <xsl:analyze-string regex="(!!(.*?)!!)" select="$text11">
            <xsl:matching-substring>
                <note>
                    <xsl:value-of select="regex-group(2)"/>
                </note>
    ...
    <xsl:template name="nd">
        ...
        <xsl:analyze-string regex="\{{ND\}}" select="$text12">
            <xsl:matching-substring>
                <nd/>
            </xsl:matching-substring>
            <xsl:non-matching-substring>
