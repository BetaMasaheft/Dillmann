...
declare variable $app:searchphrase := request:get-parameter("q", ());
...
declare function app:personslist($node as element(), $model as map(*)) {
  if (contains(sm:get-user-groups(sm:id()//sm:real/sm:username/string()), "lexicon")) then (
    ...
    let $hits :=
      for $BMperson in subsequence($BMpersons, 1, 50)
      return $BMperson
    return map {"hits": $hits}
  ) else (
    "sorry, this page is only for editors"
  )
};

declare %templates:wrap %templates:default("start", 1) %templates:default("per-page", 50) function app:persRes(
  $node as node(),
  $model as map(*),
  $start as xs:integer,
  ...
  let $names :=
    for $name in $pers/tei:persName[@xml:lang = "gez"][not(@type = "normalized")]
    return $name
  ...
  let $hits :=
    ...
    return $hit
  return <div class="w3-row-padding reference ">
    <div class="w3-quarter">
      <div class="w3-half"><a href="/{ data($id) }" target="_blank">{ string($id) }</a></div>
      <div class="w3-half">{ string-join($names, ", ") }</div>
    </div>
    <div class="w3-threequarter">
      {
        if (count($hits) gt 1) then (
          <div class="w3-container w3-margin-64">
            <div class="w3-quarter">We have found this entry in Dillmann: {
                for $hit in $hits
                let $hitID := string(root($hit)//tei:entry/@xml:id)
                return (<a href="/Dillmann/lemma/{ $hitID }" target="_blank">{ $hit }</a>, ", ")
              }
            </div>
            <div
              class="w3-threequarter w3-panel w3-red"
            >If you do not think any of the hits matches this entry you can create a new personal name entry.
               <form
                action="/Dillmann/edit/save-new-entity.xql"
                class="form-inline"
                id="createnew"
                method="post"
              >
                <div class="form-group">
                  <input class="form-control" hidden="hidden" id="form" name="form" value="{ $mainName }" />
                </div>
                <div class="form-group">
                  <input class="form-control" hidden="hidden" id="sourceen" name="sourceen" value="traces" />
                </div>
                <div class="form-group">
                  <textarea class="form-control" id="senseen" name="senseen">
                    { "<Sen< \*gez*" || $mainName || "\* [[ +n.pr.+ ]] {ND} >S>" }
                  </textarea>
                </div>
                <div class="form-group">
                  <textarea class="form-control" id="msg" name="msg" required="required">
                    { "added persName from list of persons with a normalized Gǝʿǝz name in Beta Maṣāḥǝft" }
                  </textarea>
                </div>
                <button class="btn btn-primary" type="submit">create new entry</button>
              </form>
            </div>
          </div>
        ) else
          <div class="w3-container w3-margin">
            ...
            <div
              class="w3-threequarter w3-panel w3-lightyellow w3-card-2"
            >You can create here a new personal name entry for this. Check, edit and submit.
               <form
                action="/Dillmann/edit/save-new-entity.xql"
                class="form-inline"
                id="createnew"
...
declare function app:userPage($node as element(), $model as map(*)) {
  ...
  return if ($username = "guest" or $username = "") then (
    <div class="container-fluid w3-container w3-margin" id="content">
      <p
      >Did you just arrive here by mistake or do you want to know what people look at on ths webiste? You can ask the <a
          href="/getinvolved.html"
        >team behind the Lexicon</a>.</p>
    </div>
  ) else if (
    ($username = sm:id()//sm:real/sm:username/string()) or sm:is-dba(sm:id()//sm:real/sm:username/string())
  ) then
    <div class="container-fluid w3-container w3-margin" id="content">
      <h2>
        {
          if ($username != sm:id()//sm:real/sm:username/string()) then
            "Dear " || sm:id()//sm:real/sm:username/string() || " you see this because you are admin. --->"
          else (
          )
        }
      ...
      <div class="w3-container w3-margin">
        <div class="w3-container w3-margin alert alert-success">
          ...
          <p>
            ...
            {
              let $groups :=
                for $g in sm:get-user-groups($username)
                return $g
              return string-join($groups, ", ")
            }
          </p>
          {
            for $x in sm:get-account-metadata-keys($username)
            return <p>
              <b>
                {
                  switch ($x)
              ...
              { sm:get-account-metadata($username, $x) }
            </p>
          }
        </div>
        ...
        <div class="w3-half w3-panel w3-lightygreen w3-card-2">
          {
            ...
            return (
              <h3>Your made {
                  count($changes)
                } changes in these files after the last conversion of the data from the original txt (19.4.2017).</h3>,
              <div class=" w3-container w3-margin userpanel">
                <table class="w3-table w3-hoverable">
                  <thead><tr><th>item</th><th>date and time</th><th>change</th></tr></thead>
                  <tbody>
                    {
                      ...
                      return (
                        <tr style="font-weight:bold;  border-top: 4px solid #5bc0de">
                          <td>
                            <a href="/Dillmann/lemma/{ string($ID) }">{ $form }</a>
                            {
                              if (root($ID)//tei:nd) then (
                                " ", <label class="w3-tag w3-lightgreen">NEW</label>
                              ) else (
                              )
                            }
                          </td>
              ...
              <tbody>
                {
                  let $selection :=
                    for $c in
                      collection("/db/apps/gez-en/log/")//l:logentry[l:user[. = $username]][not(
                        l:type[. = "query"]
                      )][not(contains(l:type, "XPath"))]
                  ...
                  return <tr>
                    ...
                    <td><a href="{ $loggedentity/l:url/text() }">{ $loggedentity/l:url/text() }</a></td>
                  </tr>
                }
              </tbody>
            </table>
    ...
    <div class="container-fluid w3-container w3-margin" id="content">
      <p
      >Thank you very much for you interest in user data! You are either not logged in or you are trying to look somebody else data.
...
declare function app:latestchanges($node as node(), $model as map(*)) {
  let $selection :=
    for $c in
      collection("/db/apps/gez-en/log/")//l:logentry[l:type[. = "updated"] or
        l:type[. = "backup"] or
        l:type[. = "created"] or
        l:type[contains(., "delete")]][not(l:user = "Pietro")]
    order by $c/@timestamp descending
    return $c
  return (
    <input
      ...
      placeholder="Search in the type"
      type="text" />,
    <table class="w3-table w3-hoverable" id="latestchangestable">
      <thead>
        <tr><th>type of event</th><th>date</th><th>user</th><th>lemma</th><th>description of change</th></tr>
      </thead>
      <tbody>
        {
          ...
          let $urlid := if (matches($logurl, "L[\w\d]{32}")) then (
            switch ($type)
              case "delete confirmation requested" return
                substring-after($logurl, "/Dillmann/lemma/")
              case "updated" return
                "/Dillmann/lemma/" || $logurl/text()
              default return
                $logurl/text()
          ) else
            $logurl/text()
...
declare function app:logging($node as node(), $model as map(*)) {
  ...
  return log:add-log-message($logurl, sm:id()//sm:real/sm:username/string(), "page")
};
  ...
  return (
    <div class="w3-container w3-margin">
      <div class="form-group erweit  w3-quarter">
        ...
        <div id="languages">
          <label class="switch">
            <input name="notlang" type="checkbox" />
            <div class="slider round" data-toggle="tooltip" title="not" />
          </label>
          <br />
          <input class="w3-check" name="languages" type="checkbox" value="ar" />Arabic<br />
          <input class="w3-check" name="languages" type="checkbox" value="grc" />Greek<br />
          <input class="w3-check" name="languages" type="checkbox" value="cop" />Coptic<br />
          <input class="w3-check" name="languages" type="checkbox" value="la" />Latin<br />
          <input class="w3-check" name="languages" type="checkbox" value="syr" />Syriac<br />
          <input class="w3-check" name="languages" type="checkbox" value="he" />Hebrew<br />
        </div>
      </div>
      <div class="form-group erweit  w3-quarter">
        ...
        <div id="case">
          <label class="switch">
            <input name="notcase" type="checkbox" />
            <div class="slider round" data-toggle="tooltip" title="not" />
          </label>
          <br />
          <input class="w3-check" name="case" type="checkbox" value="accusativus" />accusativus<br />
          <input class="w3-check" name="case" type="checkbox" value="dativus" />dativus<br />
        </div>
      </div>
      <div class="form-group erweit w3-quarter">
        ...
        <div id="gender">
          <label class="switch">
            <input name="notgen" type="checkbox" />
            <div class="slider round" data-toggle="tooltip" title="not" />
          </label>
          <br />
          <input class="w3-check" name="case" type="checkbox" value="f" />femininus<br />
          <input class="w3-check" name="case" type="checkbox" value="m" />masculinus<br />
        </div>
      </div>
      <div class="form-group erweit w3-quarter">
        ...
        <div id="gender">
          <label class="switch">
            <input name="notsubc" type="checkbox" />
            <div class="slider round" data-toggle="tooltip" title="not" />
          </label>
          <br />
          <input class="w3-check" name="subc" type="checkbox" value="I,1" />I,1<br />
          <input class="w3-check" name="subc" type="checkbox" value="I,2" />I,2<br />
          <input class="w3-check" name="subc" type="checkbox" value="I,3" />I,3<br />
          <input class="w3-check" name="subc" type="checkbox" value="II,1" />II,1<br />
          <input class="w3-check" name="subc" type="checkbox" value="II,2" />II,2<br />
          <input class="w3-check" name="subc" type="checkbox" value="II,3" />II,3<br />
          <input class="w3-check" name="subc" type="checkbox" value="III,1" />III,1<br />
          <input class="w3-check" name="subc" type="checkbox" value="III,2" />III,2<br />
          <input class="w3-check" name="subc" type="checkbox" value="III,3" />III,3<br />
          <input class="w3-check" name="subc" type="checkbox" value="IV,1" />IV,1<br />
          <input class="w3-check" name="subc" type="checkbox" value="IV,2" />IV,2<br />
          <input class="w3-check" name="subc" type="checkbox" value="IV,3" />IV,3<br />
        </div>
      </div>
    </div>,
    <div class="w3-container w3-margin">
      <div class="form-group erweit w3-half">
        ...
        <div id="PoS">
          <label class="switch">
            <input name="notpos" type="checkbox" />
            <div class="slider round" data-toggle="tooltip" title="not" />
          </label>
          <br />
          {
            for $x in distinct-values($collection//tei:pos/@expand)
            return (<input class="w3-check" name="pos" type="checkbox" value="{ $x }" />, $x, <br />)
          }
        </div>
      </div>
      <div class="form-group erweit  w3-half">
        ...
        <div id="lbl">
          <label class="switch">
            <input name="notlbl" type="checkbox" />
            <div class="slider round" data-toggle="tooltip" title="not" />
          </label>
          <br />
          {
            for $x in distinct-values($collection//tei:lbl/@expand)
            return (<input class="w3-check" name="lbl" type="checkbox" value="{ $x }" />, $x, <br />)
          }
        </div>
      </div>
    </div>
...
declare %templates:wrap %templates:default("lang", "la") %templates:default("mode", "cit") function app:quotes(
  $node as element(),
  $model as map(*),
  $letter as xs:string?,
  $lang as xs:string?,
...
The letters are available as buttons on the side bar and when clicked will reload the page with that parameter. :)
  let $starts-with := if ($letter) then (
    '[starts-with(.,"' || $letter || '")]'
  ) else (
  )
  let $data-collection := "/db/apps/DillmannData"
  ...
  let $langs := if ($mode = "foreign") then (
    distinct-values($collection//tei:foreign/@xml:lang)
  ) else
    distinct-values($collection//tei:cit/@xml:lang)
  (: selects among cit elements with the selected language, by default latin as it is majoritary :)
  let $translations := if ($mode = "foreign") then (
    $collection//tei:foreign[@xml:lang = $lang]
  ) else
    $collection//tei:cit[@type = "translation"][@xml:lang = $lang]
  (: build the query with the letter parameter :)
  let $query := if ($mode = "foreign") then (
    "$translations" || $starts-with
  ) else
    "$translations/tei:quote" || $starts-with

  let $trans :=
    for $word at $p in util:eval($query)
  ...
  return map {"hits": $trans, "langs": $langs}
};

(: takes map result from app:quotes and prints a navbar with buttons to filter results and the selected results :)
declare %templates:wrap %templates:default("start", 1) %templates:default("per-page", 50) function app:reverse(
  $node as element(),
  $model as map(*),
  $start as xs:integer,
  $per-page as xs:integer
      ...
      <div class="w3-bar-block w3-margin">
        {
          ...
          group by $f := $first
          return <a class="w3-bar-item w3-button w3-blue" href="?start=1&amp;letter={ $f }{ $mpar }{ $lpar }">
            { $f }
            <span class="w3-badge w3-right w3-margin-right">{ count($hit) }</span>
          </a>
        }
    ...
    <div class="w3-threequarter">
      {
        if (count($data) lt 1) then (
          <div class="w3-panel w3-card-2">Please select a value on the side bar.</div>
        ) else
                  ...
                  <div class="w3-bar-block">
                    {
                      ...
                      let $term-name :=
                        let $terms := $entry//tei:form/tei:foreign/text()
                        return if (count($terms) gt 1) then
                          string-join($terms, " et ")
                        else
                          $terms

                      return <a class="w3-bar-item" href="/Dillmann/lemma/{ $r }" target="blank">{ $term-name }</a>
                    }
                  </div>
                </div>
              </div>
...
declare function app:download($node as element(), $model as map(*)) {
  ...
  let $filename := concat(
    "Dillmann_Lexicon_",
    format-dateTime(current-dateTime(), "[Y,4][M,2][D,2][H01][m01][s01]"),
    ".txt"
  )
  let $filecontent :=
    for $d in $config:collection-root
  ...
  return <a
    ...
    id="downloaded"
  ><i aria-hidden="true" class="fa fa-download" /> Download all Dillmann Lexicon as txt file</a>
};

(: this allows a user to create an account. it is actually usable but no link points here :)
declare function app:createaccount($node as element(), $model as map(*)) {
  <div class="container">
    <form action="/Dillmann/accounts/createaccount.xql" class="form">
      <label for="n"><b>Full Name</b></label>
      <input class="form-control" id="n" name="name" placeholder="Enter your name" required="required" type="text" />
      <label for="u"><b>User Name</b></label>
      <input class="form-control" id="u" name="usr" placeholder="Choose a User name" required="required" type="text" />
      <label for="e"><b>Email</b></label>
      <input class="form-control" id="e" name="email" placeholder="enter your email" required="required" type="email" />
      <label for="p"><b>Password</b></label>
      <input class="form-control" id="p" name="psw" placeholder="Enter Password" required="required" type="password" />
...
declare function app:login($node as element(), $model as map(*)) {
  if (sm:id()//sm:username/text() = "guest") then
    <div class="w3-bar-item w3-hide-small w3-dropdown-hover " id="logging" style="margin:0;padding:0">
      <button class="w3-button w3-red w3-bar-item">Login <i class="fa fa-caret-down" /></button>
      <div class="w3-dropdown-content w3-bar-block w3-card-4">
        <form accept-charset="UTF-8" class="w3-container" id="login-nav" method="post" role="form">
    ...
    <form
      ...
      action=""
      class="w3-bar-item w3-hide-smal"
      id="logout-nav"
      method="post"
      role="form"
...
declare function app:greetings($node as element(), $model as map(*)) {
  <a href="/Dillmann/user/{ sm:id()//sm:real/sm:username/string() }" target="_blank">Hi {
...
declare function app:pdf-link($id) {
  <a xmlns="http://www.w3.org/1999/xhtml" class=" w3-bar-item w3-button w3-pale-blue" href="{ $id }.pdf">{ "pdf" }</a>
};
...
declare function app:getXML($id) {
  ...
  ><i aria-hidden="true" class="fa fa-download" />TEI/XML</a>
};
...
declare function app:bmbutton($node as element(), $model as map(*)) {
  if (contains(sm:get-user-groups(sm:id()//sm:real/sm:username/string()), "lexicon")) then (
    <a class="w3-bar-item w3-button  w3-hide-small" href="https://betamasaheft.eu/" target="_blank">Beta maṣāḥǝft</a>
  ) else (
  )
};
...
declare function app:tutorial($node as element(), $model as map(*)) {
  if (contains(sm:get-user-groups(sm:id()//sm:real/sm:username/string()), "lexicon")) then (
    <a
      ...
      href="https://betamasaheft.eu/Dillmann/tutorialextended.html"
      target="_blank"
    >Tutorial</a>
  ) else (
    <a
      class="w3-bar-item w3-button  w3-hide-small"
      href="https://betamasaheft.eu/Dillmann/tutorial.html"
      target="_blank"
    >Tutorial</a>
  )
};
  ...
  <form action="" class="w3-container" id="GtC">
    <div class="w3-bar">
      <input
        ...
        class="w3-input w3-border w3-bar-item"
        id="columnnumber"
        name="GtC"
        type="number" />
...
declare function app:editineXide($id as xs:string, $sources as node()) {
  ...
  return if (contains($groups, "lexicon") or contains($groups, "dba")) then (
    let $ss :=
      for $source in $sources/source
      return "&amp;source" || $source/@lang || "=" || substring-after($source/@value, "#")
    let $sourcesparam := string-join($ss, "")
    (: let $base := base-uri($config:collection-root//id($id)) :)
    return <a
      class="w3-button w3-pale-blue"
      href="/Dillmann/update.html?id={ $id }&amp;new=false{ $sourcesparam }"
    >Update</a>
  (: ,
...
declare function app:queryinput($node as node(), $model as map(*), $q as xs:string*) {
  <input
    class="w3-twothird w3-input w3-border diacritics"
    name="q"
  ...
  <div aria-multiselectable="true" class="panel-group" id="accordion" role="tablist">
    {
      ...
      let $n := if (request:get-parameter("new", ())) then (
        "[descendant::tei:nd]"
      ) else (
      )
      let $t := if (request:get-parameter("traces", ())) then (
        '[descendant::tei:sense[@source = "#traces"]]'
      ) else (
      )
      let $query := "$c//tei:entry" || $n || $t
      return for $term in util:eval($query)

        group by $first-letter := substring($term/tei:form[1]/tei:foreign[1], 1, 1)
        order by $first-letter
        return <div class="panel panel-default">
          <div class="panel-hading" id="{ data($first-letter) }list" role="tab">
            <h3 class="mb-0" style="text-align: center;">
              <a
                aria-controls="{ data($first-letter) }letter"
                aria-expanded="false"
                data-parent="#accordion"
                data-toggle="collapse"
                href="#{ data($first-letter) }letter"
              >{ $first-letter }</a>
              <span class="badge">{ count($term) }</span>
            </h3>
            <div
              aria-labelledby="{ data($first-letter) }list"
              class="collapse"
              id="{ data($first-letter) }letter"
              role="tabpanel"
            >
                    ...
                    <div class="card-block" id="{ data($t/@xml:id) }">
                      <h4 class="card-title"><a href="lemma/{ data($t/@xml:id) }">{ $hom || " " }{ $term-name }</a></h4>
                      {
                        if (request:get-parameter("new", ())) then (
                          ...
                          <div class="card-text">
                            {
                              if ($t/tei:sense[@xml:lang = $lang]//tei:cit) then
                                let $citLa :=
                                  let $sl := $t/tei:sense[@xml:lang = $lang]
                                  return for $cit in $sl//tei:cit
                                    return $cit
                                return ": " || string-join($citLa, ", ")
                              else (
                              )
                            }
                          </div>
                      ...
                      <div class="btn-group">
                        { app:deleteEntry($id) }
                        {
                          app:editineXide(
                            ...
                            <sources>
                              {
                                for $s in $t/tei:sense
                                return <source lang="{ $s/@xml:lang }" value="{ $s/@source }" />
                              }
                            </sources>
                          )
                        }
  ...
  <div class="table-responsive" id="abbreviations">
    <table class="table table-hover">
      ...
      <tbody>
        {
          let $refs :=
            for $r2 in $config:collection-root//tei:ref[@cRef]
          ...
          order by $ref
          return <tr id="{ replace($abbreviation/reference, " ", "") }">
...
declare function app:citations($node as node(), $model as map(*)) {
  ...
  return <div class="w3-container w3-margin" id="citationsList">
    {
      for $reference in $c//tei:ref[@cRef]
      let $trimmedr := replace(replace(string($reference/@cRef), "\s+$", ""), "^\s+", "")
      group by $ref := $trimmedr
      order by $ref
      return let $abbr := $abbreviaturen//abbreviatur[reference = $ref]
        return <div class="row">
          <h3 class="mb-0">
            { $ref } : {
              if ($abbr) then (
                if ($abbr/dillmanExplanation/text()) then
                  $abbr/dillmanExplanation/text()
                else
                  "this citation is in the abbreviation list, but without explanation"
              ) else
                "!!! not able to find explanation in abbreviation list !!!"
            }
          </h3>
          <div>
            {
              ...
              return <span>
                { $r/text() } in <a href="lemma/{ data(root($r)//tei:entry/@xml:id) }">
                  { root($r)//tei:form[1]/tei:foreign[1] }
                </a>;  </span>
            }
          </div>
        </div>
    ...
    <ul>
      {
        ...
        return <li>{ ($author || " " || $change/text() || " on " || format-date($time, "[D].[M].[Y]")) }</li>
      }
    </ul>
  </div>
};
               ...
               
{
      ...
      <idno type="url">{ ($config:appUrl || "/Dillmann/lemma/" || string(root($term)//tei:entry/@xml:id)) }</idno>
      <date type="accessed">, accessed on { format-date(current-date(), "[Y0001]-[M01]-[D01]") }</date>
    </ul>
  </div>
};
...
declare function app:itemcontent($id, $viewtype) {
  ...
  let $term := $col//id($id)
  let $hom := if ($term//tei:form/tei:seg[@type = "hom"]) then
    concat($term//tei:form/tei:seg[@type = "hom"]/text(), " ")
  else (
  )
  let $rootline := <span data-value="{ $id }" id="rootmembers" />
  let $n := data($term/@n)
  ...
  let $NE2 := string($ne2)
  let $PR2 := string($pr2)
  let $column := if ($term//tei:cb) then
    string(($term//tei:cb/@n)[1])
  else if ($term//tei:ref[@type = "hacohen"]) then
    string(($term//tei:ref[@type = "hacohen"]/@n)[1])
  else
    string(max($col//tei:cb[xs:integer(ancestor::tei:entry/@n) <= xs:integer($n)][@xml:id]/@n))
  let $next := if ($col//tei:entry[@n = $NE]/@xml:id) then
    string($col//tei:entry[@n = $NE]/@xml:id)
  else
    string($col//tei:entry[@n = $NE2]/@xml:id)
  (: <button class="highlights btn btn-sm btn-info">Highlight/Hide strings matching the words in your search</button> :)
  let $prev := if ($col//tei:entry[@n = $PR]/@xml:id) then
    string($col//tei:entry[@n = $PR]/@xml:id)
  else
    string($col//tei:entry[@n = $PR2]/@xml:id)
  return (
        ...
        <div class="w3-row">
          <div class="w3-third w3-bar">
            <span class="w3-bar-item w3-xlarge">
              { $hom }
              <span id="lemma">
                <a href="/Dillmann/lemma/{ $id }" target="_blank">
                  {
                    let $terms := root($term)//tei:form/tei:foreign/text()
                    return if (count($terms) gt 1) then
                      string-join($terms, " et ")
                    else
                      $terms
                ...
                <div class="w3-bar-item ">
                  <label class="switch highlights">
                    ...
                    <div
                      class="slider round"
                      data-toggle="tooltip"
                      title="Show or Hide highlights of each element in the entry (lemma excluded!) containing your query as a string.
  This might be different from the hits on the left, but it should help you to find your match faster." />
                  </label>
                </div>
              else (
              )
          ...
          <div class="w3-bar downloadlinks w3-twothird">
            <div class="w3-bar-item">
              {
                if ($term//tei:nd) then (
                  <span class="w3-badge w3-green w3-bar-item">New</span>
                ) else (
                  <span class="w3-badge w3-gray ">
                    <a
                      href="{
                        concat(
                          "http://www.tau.ac.il/~hacohen/Lexicon/pp",
                          format-number(
                            if (xs:integer($column) mod 2 = 1) then
                              if ($term//tei:cb) then (
                                xs:integer($column) - 2
                              ) else
                                $column
                    ...
                    >
                      ...
                      {
                        if ($term//tei:cb) then (
                          string(number(format-number($column, "#")) - 1) || "/" || format-number($column, "#")
                        ) else (
            ...
            { app:pdf-link($id) }
            { app:getXML($id) }
            <div class="w3-right">{ app:deleteEntry($id) }</div>
          </div>
        </div>
        <a
          class="smallArrow prev"
          href="{
            if ($viewtype = "home") then (
              "?" || $params || "&amp;id=" || $prev
            ) else
        ...
        >
          ...
          <span class="navlemma">{ $col//id($prev)//tei:form/tei:foreign/text() }</span>
        </a>
        { " | " }
        <a
          class="smallArrow next"
          data-value="{ $next }"
          href="{
            if ($viewtype = "home") then (
              "?" || $params || "&amp;id=" || $next
        ...
        </a>
        {
          ...
          return <div class="w3-panel entry">
            <h3>
              {
                if ($sense/@source = "#traces") then
                  "TraCES*"
                else
                  "Dillmann"
              }
              {
                if ($sense/@source) then (
                  let $s := substring-after($sense/@source, "#")
                  return <a class="w3-tooltip" href="#">
                    <i aria-hidden="true" class="fa fa-info-circle" />
                    <span class="w3-text">
                      { root($term)//tei:sourceDesc//tei:ref[@xml:id = $s]//text() } and follow-up projects, {
                        switch ($sense/@xml:lang)
                          case "la" return
                            "Latin"
                          case "ru" return
                            "Russian"
        ...
        <div class="w3-bar">
          ...
          <a
            ...
            href="mailto:info@betamasaheft.eu?Subject=[Dillmann]%20{ $id }"
            target="_blank"
          ><i aria-hidden="true" class="fa fa-envelope-o" /></a>
          <a class="w3-bar-item w3-button w3-pale-green" onclick="togglElements('cite')">Cite this lemma</a>
        </div>
        { app:revisions($term) }
        { app:cite($term) }
        <div />
      </div>
      <div class="w3-rest"><div id="showroot" /></div>
    </div>,
    <div class="w3-panel">
...
declare function app:showitem($node as node()*, $model as map(*), $id as xs:string?) {
  if ($id) then (
    ...
    <div
      class="w3-panel w3-card-4 w3-sand w3-margin w3-paddgin-64"
    >Search and click on a search result to see it here. You will be able to click on words, browse the previous and next entries, get the result on its own page and see related finds from Beta maṣāḥǝft.</div>
  )
};
...
declare function app:tempNew($node as node()*, $model as map(*)) {
  <div class="w3-container w3-margin">
    ...
    <div class="w3-half">
      <p
      >Here is a template for a new english entry with an english translation, which you can copy and paste in the editor.</p>
      <p>
        <pre
        >
                &lt;Sen&lt;
...
declare function app:newForm($node as node()*, $model as map(*)) {
  <form action="/Dillmann/edit/save-new-entity.xql" class="w3-container w3-padding" id="createnew" method="post">
    <div class="w3-container">
      <label class="w3-quarter" for="form">Lemma</label>
      <div class="w3-threequarter w3-bar">
        <input
          ...
          required="required"
          value="{
            if (request:get-parameter("form", ())) then
              request:get-parameter("form", ())
            else (
            )
          }" />
        <select class="w3-bar-item w3-select w3-border" id="formlang" name="formlang" required="required">
          <option selected="selected" value="gez">Gǝʿǝz</option>
    ...
    <div class="w3-container">
      <label class="w3-quarter" for="form">Lemma</label>
      <div class="w3-threequarter" id="checkifitalreadyexists">
        <div
          class="w3-panel w3-lightygreen w3-card-2"
        >Please paste or write something above and I will tell you if it is already in.</div>
      </div>
    </div>
    <div class="w3-container">
      <label class="w3-quarter" for="source">Source</label>
      <div class="w3-threequarter w3-bar">
    ...
    <div class="w3-container">
      <label class="w3-quarter" for="senseen">Sense</label>
      <div class="w3-threequarter">
        { app:buttons("en") }
        <textarea class="w3-input w3-border" id="senseen" name="senseen" style="height:250px;">
          {
            if (request:get-parameter("senseen", ())) then
              request:get-parameter("senseen", ())
            else (
              "<Sen<   {ND} >S>"
            )
          }
    ...
    <div class="w3-container w3-card-2 w3-margin w3-padding-64">
      <label class="w3-quarter" for="msg">What have you done?</label>
      <div class="w3-threequarter">
        <textarea class="w3-input w3-border" id="msg" name="msg" required="required">
          {
            if (request:get-parameter("msg", ())) then
              request:get-parameter("msg", ())
            else (
            )
          }
        </textarea>
      ...
      <div class="w3-container">
        <div class="w3-quarter">
          <button class="w3-button w3-green" id="confirmcreatenew" type="submit">Confirm (or lose all)</button>
        </div>
        <!--        <div class="w3-threequarter">
...
declare function app:updateFormGroup($sense) {
  ...
  let $source := if ($sense/@source) then
    string($sense/@source)
  else
    " (" || app:switchLangName($sense) || ")"
  return <div>
    <div class="w3-container">
      <label class="w3-quarter" for="source{ $lang }">Source of { "Sense" || $source }</label>
      <div class="w3-threequarter w3-bar">
        <select class="w3-select w3-border w3-bar-item" id="source{ $lang }" name="source{ $lang }" required="required">
          <option value="dillmann">
            {
              if ($parexistingsource = "dillmann") then (
          ...
          <option value="traces">
            {
              if ($parexistingsource = "traces") then (
                attribute selected { "selected" }
              ) else (
              )
            }
    ...
    <div class="w3-container">
      <label class="w3-quarter" for="sense{ $lang }">{ "Sense" || $source }</label>
      <div class="w3-threequarter">
        { app:buttons($lang) }
        <div id="wrap">
          <textarea class="w3-input w3-border" id="sense{ $lang }" name="sense{ $lang }" style="height:250px;">
            {
              if (request:get-parameter($paramname, ())) then
                request:get-parameter($paramname, ())
              else
                transform:transform($sense, "xmldb:exist:///db/apps/gez-en/xslt/xml2editor.xsl", ())
            }
          </textarea>
        </div>
    ...
    <a class="w3-button w3-small w3-red" href="#">Remove { $lang } meaning permanently</a>
  </div>
};
...
declare function app:update($node as node()*, $model as map(*)) {
  ...
  return (
    ...
    <div class="w3-panel w3-card-2">
      <p>Hi { sm:id()//sm:real/sm:username/string() }! You are updating {
          $file//tei:form/tei:foreign/text()
        }, that is great!</p>
      <p> Please follow the data entry support on the side of this form for editing the entries.</p>
      <p
      > Remember, you are here editing the dictionaries as sources of information, not annotating texts. The structure given to the entries is useful for many purposes.</p>
    </div>,
    <form
      action="/Dillmann/edit/edit.xq"
      class="w3-container w3-padding input_fields_wrap"
      id="updateEntry"
      method="post"
    >
      <input hidden="hidden" name="id" value="{ $id }" />
      <div class="w3-container">
        <label class="w3-quarter col-form-label" for="form">Lemma</label>
        <div class="w3-threequarter w3-bar">
          <div class="w3-bar-item w3-bar">
            <input
              class="w3-input w3-border w3-bar-item diacritics"
              id="senselemma"
              name="form"
              value="{ $file//tei:form/tei:foreign/text() }" />
          ...
          <input class="w3-check w3-bar-item" id="rootCheck" name="root" type="checkbox" value="root">
            {
              if ($file//tei:form/tei:rs[@type]) then
...
declare function app:buttons($name) {
  <div class="w3-bar">
    <a class="w3-button w3-xsmall w3-blue" id="{ $name }NestSense">Meaning</a>
    <a class="w3-button w3-xsmall w3-blue" id="{ $name }translation">Translation</a>
    <a class="w3-button w3-xsmall w3-blue" id="{ $name }transcription">Transcription</a>
    <a class="w3-button w3-xsmall w3-blue" id="{ $name }PoS">PoS</a>
    <a class="w3-button w3-xsmall w3-blue" id="{ $name }reference">Reference</a>
    <a class="w3-button w3-xsmall w3-blue" id="{ $name }bibliography">Bibliography</a>
    <a class="w3-button w3-xsmall w3-blue" id="{ $name }otherLanguage">Language</a>
    <a class="w3-button w3-xsmall w3-blue" id="{ $name }internalReference">Internal Reference</a>
    <a class="w3-button w3-xsmall w3-blue" id="{ $name }gramGroup">Grammar Group</a>
    <a class="w3-button w3-xsmall w3-blue" id="{ $name }label">Label</a>
    <a class="w3-button w3-xsmall w3-blue" id="{ $name }case">Case</a>
    <a class="w3-button w3-xsmall w3-blue" id="{ $name }gen">Gender</a>
    <a class="w3-button w3-xsmall w3-blue" id="{ $name }ND">ND</a>
  </div>
};
...
declare function app:upconvertSense($senseAndSource) as node() {
  ...
  let $newsense := transform:transform(
    ...
    <parameters><param name="source" value="{ $newTextsource }" /></parameters>
  )
  return $newsense
};
...
declare function app:DoUpdate($node as node()*, $model as map(*)) {
  ...
  let $backupfilename := (
    $id || "BACKUP" || format-dateTime(current-dateTime(), "[Y,4][M,2][D,2][H01][m01][s01]") || ".xml"
  )
  ...
  } catch * { util:log("warn", "Could not store backup file: " || $err:description)() }

  return if (contains($parametersName, "sense")) then (
    let $eachsense := <senses>
      {
        for $parm in $parametersName
        return if (starts-with($parm, "sense")) then (
          let $couple := <couple>
            <sense>{ request:get-parameter($parm, ()) }</sense>
            <source>{ request:get-parameter(("source" || substring-after($parm, "sense")), ()) }</source>
          </couple>
          return app:upconvertSense($couple)
        ) else (
        )
      }
    </senses>
    ...
    let $temporary :=
      ...
      return <TEI xmlns="http://www.tei-c.org/ns/1.0" xml:lang="la">
        <teiHeader>
          <fileDesc>
            <titleStmt><title xml:lang="gez">{ $form }</title><author>Alessandro Bausi</author></titleStmt>
            <publicationStmt>
              ...
              <availability>
                <licence
                  target="http://creativecommons.org/licenses/by-sa-nc/4.0/"
                >
            ...
            <sourceDesc>
              <p>The origin of the lexicon is in a thoroughly elaborated txt version of <ref
                  target="https://archive.org/details/lexiconlinguaeae00dilluoft"
                  xml:id="dillmann"
                >Dillmann,
                  Christian Friedrich August. <emph
              ...
              <p>Additional records were created by the project <ref
                  target="https://www.traces.uni-hamburg.de/"
                  xml:id="traces"
                >
                  TraCES (ERC Advanced Grant, Grant Agreement 338756)</ref> and other follow-up projects</p>
              <p>Sources include also <ref
                  target="https://archive.org/details/supplmentaulexic0000grba"
                  xml:id="grebaut"
                >Sylvain Grébaut, Supplément au lexicon linguae aethiopicae de August Dillmann: (1865) et édition du lexique de Juste d'Urbin (1850 - 1855), Paris Imprimerie Nationale 1952</ref>
              </p>
            </sourceDesc>
          </fileDesc>
          <encodingDesc><p>A digital edition of the Lexicon in TEI.</p></encodingDesc>
          <profileDesc>
            <langUsage>
          ...
          <revisionDesc>
            <change when="{ format-date(current-date(), "[Y0001]-[M01]-[D01]") }" who="AppValidationTemporaryFile">
              { $msg }
            </change>
          </revisionDesc>
        </teiHeader>
        <text>
          <body>
            <div>
              <entry>
                <form><foreign xml:lang="gez">{ $form }</foreign></form>
                {
                  for $s in $eachsense//tei:sense
                  return $s
                }
              </entry>
            </div>
    ...
    return if ($validation = true()) then (
      let $doc := doc($targetfileuri)
      let $sensesArray := $doc//tei:sense
      let $sensesLang :=
        for $lang in $sensesArray/@xml:lang
        return $lang
      let $eachLang :=
        for $lang in $eachsense//tei:sense/@xml:lang
        return $lang
      ...
      let $updateform :=
        ...
        return if ($sensesArray[@xml:lang = $slang]) then (
          update replace $sensesArray[@xml:lang = $slang] with $s
        ) else (
          update insert $s into $doc//tei:entry
        )

      let $deleteRemoved :=
        for $removedLang in distinct-values($sensesLang[not(. = $eachLang)])
        return update delete $sensesArray[@xml:lang = $removedLang]

      let $change := <change
        xmlns="http://www.tei-c.org/ns/1.0"
        when="{ format-date(current-date(), "[Y0001]-[M01]-[D01]") }"
        who="{
          switch (sm:id()//sm:real/sm:username/string())
            case "Pietro" return
              "PL"
            case "Vitagrazia" return
              "VP"
      ...
      let $sendmails := (
        let $contributorMessage := <mail>
          ...
          <message>
            <xhtml>
              <html>
                <head><title>This is a summary of the changes you made to { $filename }</title></head>
                <body>
                  ...
                  <p>
                    <a href="{ $config:appUrl }/Dillmann/lemma/{ $id }" target="_blank">See {
                        $filename
        ...
        return mail:send-email($contributorMessage, "public.uni-hamburg.de", ()),
        let $EditorialBoardMessage := <mail>
          ...
          <message>
            <xhtml>
              <html>
                <head><title>Changes in { $filename } on </title></head>
                <body>
        ...
        return mail:send-email($EditorialBoardMessage, "public.uni-hamburg.de", ())
      )
      return <div class="alert alert-success">
        <h2>{ $title }</h2>
        <p class="lead">Dear { $cU }, Lemma  <a href="/Dillmann/lemma/{ $id }">
            { $filename }
          </a> has been updated successfully!</p>
      ...
      <div class="w3-container w3-margin w3-panel w3-lightyellow w3-card-2">
        <p
          class="lead"
        >Sorry, the document you are trying to save is not valid.
  ...
  return if ($x = $match) then
    ...
    return (: loop each occurrence of that character to do the substitutions one by one in case it matches, 0 and 3 for the example :) for $i in
      ...
      return (: substitute only that occurence by removing it and adding the substitute in its place, so in the first loop, remove a and then add d before position 0 :) let $rem :=
        ...
        return ($newstring, app:repl($newstring, $match, $sub))
...
declare function app:buildqueryparts($parName, $attName, $not, $el) {
  let $parameterslist := request:get-parameter-names()
  return if (contains($parameterslist, $parName)) then
    let $lp := request:get-parameter($parName, ())
    let $lpvals :=
      for $slp in $lp
      return if ($attName = "value") then (
        "text()" || "='" || $slp || "'"
      ) else (
        "@" || $attName || "='" || $slp || "'"
      )
    let $val := if (count($lpvals) gt 1) then
      string-join($lpvals, " or ")
    else
      $lpvals
    return if (contains($parameterslist, $not)) then (
      "[not(descendant::tei:" || $el || "[" || $val || "])]"
    ) else (
      "[descendant::tei:" || $el || "[" || $val || "]]"
    )
  else (
  )
};

declare %templates:wrap %templates:default("mode", "none") function app:AdvQuery(
  $node as node()*,
  $model as map(*),
  $q as xs:string*
) {
  let $parameterslist := request:get-parameter-names()
  return if (empty($parameterslist)) then (
  ) else
    let $data-collection := "/db/apps/DillmannData"
    ...
    let $qtext := if (empty($q)) then (
    ) else
      app:substitutionsInQuery($q)
    ...
    let $query := if (empty($q) or $q = "") then (
      "$coll//tei:entry" || $c || $l || $p
    ) else (
      "$coll//tei:entry[ft:query(*, $qtext, $options)]" || $c || $l || $p
    )

    let $hits :=
      for $hit in util:eval($query)
      return $hit
    return map {"hits": $hits}
};

declare %templates:wrap %templates:default("mode", "none") function app:query(
  $node as node()*,
  $model as map(*),
  $q as xs:string?,
  $ref as xs:string?,
  ...
  return if (empty($q)) then (
    if ($cun) then (
      ...
      let $hits :=
        for $hit in $coll//tei:entry//tei:cb[@n = $formatCun]
        return $hit
    ...
    if ($mode = "none") then
      if ($ref = "ref") then (
        let $hits :=
          for $hit in $coll//tei:entry//tei:ref[@cRef = $q]
          return $hit
        return map {"hits": $hits}
      ) else if ($ref = "addenda") then (
        let $hits :=
          for $hit in $coll//tei:entry//tei:ref[contains(@target, $q)]
          return $hit
        return map {"hits": $hits}
      ) else (
        let $qtext := if (empty($q)) then (
        ...
        return let $qp := if (empty($q) or $q = "") then (
            "$coll//tei:entry" || $erw
          ) else
            "$coll//tei:entry[ft:query(*, $qtext, $options)]" || $erw

          let $hits :=
            for $hit in util:eval($qp)
            let $sorting := if (empty($q) or $q = "") then
              $hit//tei:form[1]/tei:foreign[1]/text()[1]
            else
              ft:score($hit)
            order by $sorting descending
            return $hit
          return map {"hits": $hits, "query": $qp}
      )
      ...
      let $hits :=
        ...
        order by ft:score($hit) descending
        return $hit
      return (: Process nested templates :) map {"hits": $hits, "query": $queryExpr}
    )
  )
};

declare %templates:wrap function app:advhit-count($node as node()*, $model as map(*)) {
  <h3>You found  <span xmlns="http://www.w3.org/1999/xhtml" id="hit-count">
      { count($model("hits")) }
    </span> entries!</h3>
};

declare %templates:wrap function app:hit-count($node as node()*, $model as map(*)) {
  let $q := request:get-parameter("q", ())
  return if (empty($q)) then (
  ) else
    <h3>You found "{ $q }" in <span xmlns="http://www.w3.org/1999/xhtml" id="hit-count">
        { count($model("hits")) }
      </span> entries!</h3>
};

declare %templates:wrap function app:biblio-count($node as node()*, $model as map(*)) {
  <h3>There are <span xmlns="http://www.w3.org/1999/xhtml" id="hit-count">
      { $model("total") }
    </span> distinct bibliographical sources.</h3>
};
...
function app:paginate(
  ...
  if ($min-hits < 0 or count($model("hits")) >= $min-hits) then
    ...
    let $params := string-join(
      for $param in request:get-parameter-names()
      for $value in request:get-parameter($param, ())
      return if ($param = "collection") then (
      ) else if ($param = "user") then (
      ) else if ($param = "password") then (
...
declare %templates:wrap %templates:default("start", 1) %templates:default("per-page", 10) function app:fullRes(
  $node as node(),
  $model as map(*),
  $start as xs:integer,
  $per-page as xs:integer
  ...
  let $term-name :=
    ...
    return if (count($terms) gt 1) then
      ...
      $terms
  let $hom := if (root($term)//tei:form/tei:seg[@type = "hom"]) then
    concat(root($term)//tei:form/tei:seg[@type = "hom"]/text(), " ")
  else (
  )
  order by ft:score($term) descending
  return <div class="w3-row row">
    <div class="w3-quarter">
      <div class="w3-twothird">
        <a class="w3-button w3-blue" href="?{ $params }&amp;id={ data($id) }">{ $hom || $term-name }</a>
      </div>
      ...
      <div class="w3-threequarter">
        {
          ...
          return kwic:get-summary($expanded, $match, <config width="40" />)
        }
      </div>
      <div class="w3-quarter">
        {
          (: :app:editineXide($id, <sources>{for $s in root($term)//tei:sense return <source lang="{$s/@xml:lang}" value="{$s/@source}"></source>}</sources>): :) (
          )
        }
      </div>
    </div>
...
declare %private function app:sanitize-lucene-query($query-string as xs:string) as xs:string {
  ...
  let $query-string := if (app:number-of-matches($query-string, '"') mod 2) then
    $query-string
  else
    replace($query-string, '"', " ")
  (: if there is an uneven number of parentheses, delete all parentheses. :)
  let $query-string := if (
    (app:number-of-matches($query-string, "\(") + app:number-of-matches($query-string, "\)")) mod 2 eq 0
  ) then
    $query-string
  else
...
declare function app:biblform($node as node(), $model as map(*)) {
  <form xmlns="http://www.w3.org/1999/xhtml" action="" class="w3-container">
    ...
    <div class="w3-container w3-bar">
      <button class="w3-button w3-pale-green w3-bar-item" type="submit">
        <i aria-hidden="true" class="fa fa-filter" />
      </button>
      <a class="w3-button w3-pale-green w3-bar-item" href="/Dillmann/bibl.html" role="button">
...
(:~
 ...
 :)
declare %templates:default("collection", "") %templates:default("pointer", "") function app:bibl(
  $node as node(),
  $model as map(*),
  ...
  let $bms :=
    ...
    return $bibl
  return map {"hits": $bms, "total": count($bms), "type": "bibliography"}
};

declare %templates:wrap %templates:default("start", 1) %templates:default("per-page", 10) function app:biblRes(
  $node as node(),
  $model as map(*),
  $start as xs:integer,
  $per-page as xs:integer
  ...
  return <div class="w3-container w3-margin biblio">
    <div class="biblioentry w3-half" id="{ $target }" />
    <div class="w3-half">
        ...
        <ul>
          {
            ...
            return for $lem in distinct-values(root($root)//tei:entry/tei:form/tei:foreign[1]/text())
              order by $lem
              return <li><a href="/Dillmann/lemma/{ string($root/@xml:id) }">{ $lem }</a></li>
          }
        </ul>
      </div>
  ...
  return <div class="w3-top">
    <div class="w3-bar w3-black w3-card">
      ...
      </div>
      {
        if ($user = "guest") then
          <div class="w3-dropdown-hover w3-hide-small" id="logging">
            <button class="w3-button  w3-bar-item">Login <i class="fa fa-caret-down" /></button>
            <div class="w3-dropdown-content w3-bar-block w3-card-4">
              <form accept-charset="UTF-8" class="w3-container" id="login-nav" method="post" role="form">
                <label for="user">User:</label>
                <input class="w3-input" name="user" required="required" type="text" />
      ...
      <div class="w3-dropdown-hover w3-hide-small" id="about">
        <a class=" w3-button" href="/Dillmann/user/{ $user }" target="_blank" title="about">
Hi { $user }!<i
      ...
      <div class="w3-dropdown-hover w3-hide-small" id="lists">
        ...
        <div class="w3-dropdown-content w3-bar-block w3-card-4">
          <a class="w3-bar-item w3-button  w3-hide-small w3-hide-medium" href="/Dillmann/list" id="list">Browse</a>
          <a
            class="w3-bar-item w3-button  w3-hide-small w3-hide-medium"
            href="/Dillmann/bibl.html"
            id="biblio"
          >Bibliography</a>
          <a
            class="w3-bar-item w3-button  w3-hide-small w3-hide-medium"
            href="/Dillmann/reverse"
            id="reverse"
          >Reverse Index</a>
          <a
            class="w3-bar-item w3-button  w3-hide-small w3-hide-medium"
            href="/Dillmann/abbreviations"
            id="abbreviations"
          >Abbreviations</a>
          <a
            class="w3-bar-item w3-button  w3-hide-small w3-hide-medium"
            href="/Dillmann/citations"
            id="quotes"
          >Citations</a>
          <div
            class="w3-bar-item w3-button  w3-hide-small w3-hide-medium"
            data-template="app:downloadbutton"
            id="downloads" />
        </div>
      </div>
      <a
        class="w3-bar-item w3-button  w3-hide-small w3-hide-medium"
        href="/Dillmann/getinvolved.html"
        id="getInvolved"
      >Get involved</a>
      <div class="w3-bar-item w3-button  w3-hide-small w3-hide-medium" data-template="app:tutorial" id="tutorial" />
      <div class="w3-bar-item w3-button  w3-hide-small w3-hide-medium" data-template="app:bmbutton" id="BM" />
      <a
        class="w3-bar-item w3-button  w3-hide-small w3-hide-medium"
        href="https://github.com/BetaMasaheft/Dillmann/issues/new?title=something%20is%20wrong"
      ><i aria-hidden="true" class="fa fa-exclamation-circle" /> report issue</a>
      <a class="w3-padding w3-hover-red w3-hide-small w3-right" href="/Dillmann"><i class="fa fa-search" /></a>
    </div>
  </div>
};

declare function app:modals($node as element(), $model as map(*)) {
  <div class="modal fade" id="versionInfo" role="dialog">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <p
          >        You are looking at a pre-alpha version of this website. If you are not an editor you should not even be seeing it at all. For questions <a
              href="mailto:info@betamasaheft.eu?Subject=Issue%20Report%20BetaMasaheft"
              target="_blank"
...
function app:lista(
  $node as element(),
  $model as map(*),
  $firstletter as xs:string?,
  $lang as xs:string?,
  $mode as xs:string
) {
  (: if a parameter letter is given, then construct xpath to select only the elements whose content starts with that letter.
The letters are available as buttons on the side bar and when clicked will reload the page with that parameter. :)
  let $starts-with := if ($firstletter) then (
  ...
  (: selects the cit elements, which contain translations and output distinct values of the language for the side menu :)
  let $langs := if ($mode = "foreign") then (
    distinct-values($c//tei:foreign/@xml:lang)
  ) else
    distinct-values($c//tei:cit/@xml:lang)
  (: selects among cit elements with the selected language, by default latin as it is majoritary :)
  let $translations := if ($mode = "foreign") then (
    $c//tei:foreign[@xml:lang = $lang]
  ) else
    $c//tei:cit[@type = "translation"][@xml:lang = $lang]
...
declare %templates:wrap %templates:default("start", 1) %templates:default("per-page", 50) function app:listb(
  $node as element(),
  $model as map(*),
  $start as xs:integer,
  $per-page as xs:integer
      ...
      <div class="w3-bar-block w3-margin">
        {
          ...
          order by $f
          return <a class="w3-bar-item w3-button w3-blue" href="?start=1&amp;firstletter={ $f }">
            { $f }
            <span class="w3-badge w3-right w3-margin-right">{ count($hit) }</span>
          </a>
        }
              ...
              <div class="w3-col w3-rest">
                {
                  ...
                  for $s in $sense
                  order by $s/@source
                  return transform:transform($s, "xmldb:exist:///db/apps/gez-en/xslt/text.xsl", ())
                }
              </div>
            </div>
          }
