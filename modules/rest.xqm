...
declare
  %rest:GET %rest:path("/api/Dillmann/rootmembers/{$id}") %output:method("json")
...
ORDER BY ?sequence"
  ...
  let $thisN := xs:integer(
    $thisResult//sr:binding[@name = "sequence"]/sr:literal
  )
  let $prevs :=
    for $p in
      $fusekicall//sr:result[xs:integer(
        sr:binding[@name = "sequence"]/sr:literal
      ) lt
        $thisN]
      ...
      map {
        "here":
          map {
            ...
            "n":
              xs:integer(
                $thisResult/sr:binding[@name = "sequence"]/sr:literal/text()
              ),
...
declare
  %rest:GET %rest:path("/api/Dillmann/{$lemma}/teientry") %output:method("xml")
function api:getLemma ($lemma as xs:string?) {
  ...
  return if (exists($item)) then (
      ...
      $config:response400,
      <info>{ $lemma || "is not a lemma unique id of any entry." }</info>
    )
};

declare
  %rest:GET %rest:path("/api/Dillmann/{$lemma}/json") %output:method("json")
function api:getLemmaJson ($lemma as xs:string?) {
  ...
  return if (exists($item)) then (
      ...
      $config:response400,
      map {"info": ($lemma || "is not a lemma unique id of any entry.")}
    )
};

declare
  %rest:GET %rest:path("/api/Dillmann/{$lemma}/txt") %output:method("text")
function api:getLemmaTXT ($lemma as xs:string?) {
  ...
  return if (exists($item)) then (
      ...
      transform:transform(
        $item,
        "xmldb:exist:///db/apps/gez-en/xslt/txt.xsl",
        ()
      )
    ) else (
...
function api:getHugeTXT ($start as xs:integer*, $total as xs:integer*) {
  (
    ...
    let $filecontent :=
      for $d in
        subsequence(
          $config:collection-root//tei:entry[starts-with(@xml:id, "L")],
          $start,
          $total
        )
      order by $d/@n
...
declare
  %rest:GET %rest:path("/api/Dillmann/number/{$n}") %output:method("json")
...
declare
  %rest:GET %rest:path("/api/Dillmann/column/{$n}") %output:method("json")
...
function api:getsamelemma ($lemma as xs:string*) {
  (
    $config:response200Json,
    let $eval-string := concat(
      " $config:collection-root//tei:form/tei:foreign[ft:query(.,'",
      $lemma,
      "')]"
    )
    let $hits :=
