...
declare variable $updatefuseki:lexicogEntries := for $lexicogentry in
  $config:collection-root//t:rs[@type = "root"]
...
declare variable $updatefuseki:limeEntry :=
  $config:collection-root//t:entry[not(descendant::t:rs[@type = "root"])];
...
declare function updatefuseki:sense ($parentURI as xs:string, $sense) {
  ...
  let $nested :=
    ...
    return if ($s/t:sense) then
        updatefuseki:sense($senseURI, $s/t:sense)
      else (
      )
  let $nestedLexical :=
    for $s in $sense
...
declare function updatefuseki:entry ($root, $operation) {
  ...
  return (: this part of the query is relevant only if a root is updated :) if (
      count($entryIndex) ge 1
    ) then (
      ...
      let $rootmembers :=
        ...
        return concat("dillmann:", string($member/@xml:id), "_comp")
      let $components :=
      ...
      let $entry := "dillmann:lexicon lexicog:entry  " ||
        $entryURI ||
        "_entry ;
                                      rdf:_" ||
        string($entryIndex[1]) ||
        " " ||
        $entryURI ||
        "_entry . #added, not in documentation, to sequence the roots in dillmann
                                      " ||
        $entryURI ||
        "_entry a lexicog:Entry
                                             " ||
        (
  ...
  let $entryURI := concat("dillmann:", string($id))
  let $senses :=
    for $sense in $root/t:sense
    let $senseURI := $entryURI || "_sense_" || string($sense/@xml:id)
    let $definition := if ($sense/t:cit[@type = "translation"]/t:quote) then (
      for $d in $sense/t:cit[@type = "translation"]
      group by $L := $d/@xml:lang
      let $values :=
        for $def in $d/t:quote
        return '"' || $def || '"@' || string($L)
      return "skos:definition " || string-join($values, ", ")
    ) else (
    )
    return (
  ...
  let $lemmaentry := $entryURI ||
    '_form a ontolex:Form ;
       ontolex:writtenRep "' ||
    normalize-space(string-join($root/t:form//text())) ||
    '"@gez .
   ' ||
    $entryURI ||
    " ontolex:lexicalForm  " ||
   ...
   " ||
    string-join($senses, "
")
  return fusekisparql:update("dillmann", $operation, $lemmaentry)
};
