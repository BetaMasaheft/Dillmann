...
declare variable $updatefuseki:lexicogEntries := for $lexicogentry in $config:collection-root//t:rs[@type = "root"]
...
declare function updatefuseki:entry($root, $operation) {
  ...
  return (: this part of the query is relevant only if a root is updated :) if (count($entryIndex) ge 1) then (
