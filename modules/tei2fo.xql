...
declare function fo:lang ($lang as xs:string) {
  switch ($lang)
    case "ar" return
      (
        attribute font-family { "NotoNaskhArabic" },
        attribute writing-mode { "rl" }
      )
    case "he" return
...
declare function fo:tei2fo ($nodes as node()*) {
  ...
  return typeswitch ($node)
      ...
      case element(tei:div) return
        <fo:block font-family="Ludolfus" id="{ generate-id($node) }">
          { fo:tei2fo($node/node()) }
        </fo:block>
      ...
      case element(tei:form) return
        <fo:block font-family="Ludolfus" font-size="12pt" font-weight="bold">
          { $node//text() }
        </fo:block>
      ...
      case element(tei:ref) return
        if ($node/@type = "external") then (
          <fo:basic-link external-destination="{ $node/@target }">
            { fo:tei2fo($node/node()) }
          </fo:basic-link>
      ...
      case element(tei:sense) return
        if ($node/@source or $node/@n = "L") then
          ...
          if ($node/@n = "E") then
            <fo:block
              font-weight="bold"
            >Comparative and etymological data</fo:block>
...
declare function fo:zoteroCit ($ZoteroUniqueBMtag as xs:string) {
  ...
  return replace($parseedZoteroApiResponse?1?citation, "&lt;span&gt;", "")
      => replace("&lt;/span&gt;", "")
};
          ...
          <fo:block-container>
            <fo:block font-size="0.8em" text-align="right">
              ...
              >Lexicon Linguae Aethiopicae {
                  fo:footer($entry//tei:form)
                } | PDF generated from the app on { current-dateTime() }
              </fo:basic-link>
            </fo:block>
          </fo:block-container>
        </fo:static-content>
