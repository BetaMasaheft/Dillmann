...
declare function fo:tei2fo($nodes as node()*) {
  ...
  return typeswitch ($node)
    ...
    case element(tei:div) return
      <fo:block font-family="Ludolfus" id="{ generate-id($node) }">{ fo:tei2fo($node/node()) }</fo:block>
    case element(tei:entry) return
      <fo:block font-family="Ludolfus" font-size="12pt" text-indent="1cm">{ fo:tei2fo($node/node()) }</fo:block>
    case element(tei:form) return
      <fo:block font-family="Ludolfus" font-size="12pt" font-weight="bold">{ $node//text() }</fo:block>
    ...
    case element(tei:ref) return
      if ($node/@type = "external") then (
        <fo:basic-link external-destination="{ $node/@target }">{ fo:tei2fo($node/node()) }</fo:basic-link>
      ) else if ($node/@target) then (
        <fo:inline>{ "{DiL" || replace($node/@target, "#c", ".") || "}" }</fo:inline>
      ) else
        fo:tei2fo($node/node())
    ...
    case element(tei:sense) return
      if ($node/@source or $node/@n = "L") then
        <fo:block-container margin-top="3mm">
          {
            ...
            ) else if ($node/@n = "L") then
              <fo:block font-weight="bold">Leslau</fo:block>
            else if ($node/@n = "G") then
              <fo:block font-weight="bold">Grebaut</fo:block>
            else (
            ),
        ...
        if ($node/@n = "E") then
          <fo:block font-weight="bold">Comparative and etymological data</fo:block>
        else if ($node/@n = "C") then
          <fo:block font-weight="bold">Cross-references</fo:block>
        else if ($node/@n = "X") then
          <fo:block font-weight="bold">Compounds</fo:block>
        else if ($node/@n) then (
          <fo:inline font-weight="bold">{ string($node/@n) || ")" }</fo:inline>
        ) else (
        ),
        fo:tei2fo($node/node())
      )
    ...
    <fo:page-sequence master-reference="Dillmann">
      <fo:static-content flow-name="xsl-region-before">
        <fo:block-container display-align="center" height="100%">
          <fo:block font-family="sans-serif" font-size="0.8em" text-align="center"> - <fo:page-number /> - </fo:block>
        </fo:block-container>
      </fo:static-content>
      <fo:static-content flow-name="xsl-region-after">
        <fo:block-container>
          <fo:block font-size="0.8em" text-align="right">
            <fo:basic-link
              external-destination="{ $config:appUrl }/Dillmann/lemma/{ $id }"
            >Lexicon Linguae Aethiopicae { fo:footer($entry//tei:form) } | PDF generated from the app on {
                current-dateTime()
              }
            </fo:basic-link>
          </fo:block>
      ...
      <fo:flow flow-name="xsl-region-body" font-family="Ludolfus" text-align="justify">
        ...
        <fo:block margin-top="3mm">
          <fo:block font-weight="bold">Bibliography</fo:block>
          {
            ...
            return <fo:block><fo:inline start-indent="5mm" text-indent="-5mm">{ fo:Zotero($ptr) }</fo:inline></fo:block>
          }
        </fo:block>
        <fo:block font-size="0.6em" margin-top="3mm">
          <fo:block font-weight="bold">Revisions</fo:block>
          <fo:list-block provisional-distance-between-starts="1em" provisional-label-separation="1em">
            {
...
return response:stream-binary($pdf, "media-type=application/pdf", $id || ".pdf")
