...
dillmann:lexicon a lexicog:LexicographicResource ;
	    ...
	    dc:language "en" .
	    '
  return fusekisparql:update("dillmann", $operation, $tripleslexicon)
...
let $lexicon :=
  for $entry in ($lexicogEntries, $limeEntry)
  let $entryURI := concat("dillmann:", string($entry/@xml:id))
  let $senses :=
    for $sense in $entry/t:sense
  ...
  let $entry := $entryURI ||
    '_form a ontolex:Form ;
       ontolex:writtenRep "' ||
    normalize-space(string-join($entry/t:form//text())) ||
    '"@gez .
