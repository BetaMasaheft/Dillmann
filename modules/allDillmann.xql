...
declare function fo:tei2fo($nodes as node()*) {
  ...
  return typeswitch ($node)
    ...
    case element(tei:div) return
      <fo:block font-family="Noto" id="{ generate-id($node) }">{ fo:tei2fo($node/node()) }</fo:block>
    case element(tei:entry) return
      <fo:block font-family="Noto" font-size="12pt" text-indent="1cm">{ fo:tei2fo($node/node()) }</fo:block>
    case element(tei:form) return
      <fo:inline font-family="Ludolfus" font-size="12pt" font-weight="bold">{ $node//text() }</fo:inline>
    ...
    case element(tei:ref) return
      if ($node/@target) then (
        <fo:inline>
          { "{DiL" || replace($node/@target, "#c", ".") || "}" } [↗<fo:page-number-citation
    ...
    case element(tei:bibl) return
      <fo:inline>
        ...
        { " " || $node/tei:citedRange/@unit || " " || $node/tei:citedRange/text() }
      </fo:inline>
    case element(tei:sense) return
      (
        if ($node/@source = "#traces") then (
          <fo:inline font-weight="bold">{ (" TraCES " || string($node/@xml:lang) || " ") }</fo:inline>
        ) else (
        ),
        if ($node/@n) then (
...
declare function fo:zoteroCit($ZoteroUniqueBMtag as xs:string) {
  ...
  return replace($parseedZoteroApiResponse//*:pair[@name = "citation"]/text(), "&lt;span&gt;", "")
    => replace("&lt;/span&gt;", "")
};
          ...
          return <fo:block margin-top="5mm">
            {
              if ($entry//tei:rs[@type = "root"]) then (
                <fo:leader leader-length="80%" leader-pattern="rule" rule-style="solid" rule-thickness="2pt" />
              ) else (
              )
            }
...
return response:stream-binary($pdf, "media-type=application/pdf", "DillmannAll.pdf")
