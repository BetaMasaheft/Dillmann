...
let $morph :=
  for $m in $annotation//t:fs[@type = "morpho"]/t:f[not(@name = "lex")]
...
let $tokens := if ($annotation//t:fs[@type = "tokens"]) then (
  ...
  $annoID ||
    ...
    '" ;
rdfs:seeAlso dillmann:' ||
    substring-before($annotation//t:f[@name = "lex"]/text(), "--") ||
    " ."

let $query := $annoID ||
  " a oa:Annotation ;
        oa:hasTarget bm:" ||
  string($annotation/ancestor::t:TEI/@corresp) ||
  ' ;
        rdfs:label "' ||
  $annotation//t:f[@name = "fidäl"]/text() ||
  '"@gez ;
        rdfs:label "' ||
  $annotation//t:f[@name = "translit"]/text() ||
  '"@gez-trsl . 
...
return (: $query :) fusekisparql:update("traces", $operation, $query)
