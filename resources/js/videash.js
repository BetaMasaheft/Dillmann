...
$(document).ready(function () {
  ...
  $('div.entry [lang="gez"]').each(function (wn) {
    ...
    $.each(words, function (i, v) {
      ...
      if (i == countwords - 1) {
        $(word).append(
          $(
            "<span class='alpheios-word popup' lang='gez' data-value='p" +
              wn +
              i +
              "'>" +
              nostops.w +
  ...
  $('[title="quod videas"]').each(function () {
    if ($(this).prev().is("span")) {
      ...
      var newa =
        ' <a target="_blank" href="/Dillmann/lemma/' +
        n +
        '">' +
        $(v).text() +
        " " +
        $(next).text() +
        '  <i class="fa fa-external-link-square" aria-hidden="true"/></a>';
      $(v).replaceWith(newa);
    }
  });
    ...
    if (data.prev == null) {
      ...
      if (data.prev.length >= 1) {
        for (var i = 0; i < data.prev.length; i++) {
          ...
          ptext +=
            '<li class="nodot"><a href="/Dillmann/lemma/' +
            match.id +
            '">' +
            match.lem +
            "</a> [#" +
