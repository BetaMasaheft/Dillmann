...
$(document).ready(function () {
  ...
  $("a.reference").on("click", function () {
    ...
    $.getJSON(apiDTS + reference, function (data) {
      ...
      if (data["0"].info) {
        result +=
          '<span class="RefPopup popup" data-value="' +
          reference +
          '"><i class="fa fa-file-text-o" aria-hidden="true"/><span class="w3-hide w3-small w3-padding popuptext" id="' +
          reference +
          '">Text of the passage   <span ><p>There is no text yet for this passage in Beta maṣāḥǝft.</p><a target="_blank" href="/works/' +
          bmid +
          '">See Work record</a></span></span></span>';
      } else {
        result +=
          '<span class="RefPopup popup" data-value="' +
          reference +
          '"><i class="fa fa-file-text-o" aria-hidden="true"/><span class="w3-hide w3-small w3-padding popuptext" id="' +
          reference +
          '">Text of the passage  <span  ><p>Text of ' +
          cit +
          " .</p><p>" +
          text +
          '</p><a target="_blank" href="/works/' +
          bmid +
          "/text?ref=" +
          splitref[1] +
          '">See full text</a></span></span></span>';
      }
      $(el).html(result);
    });
  });
  ...
  //console.log(lemma)
  var apiurl =
    "/api/kwicsearch?element=ab&element=title&element=q&element=p&element=l&element=incipit&element=explicit&element=colophon&element=summary&element=persName&element=placeName&q=";
    ...
    $.getJSON(call, function (data) {
      ...
      if (numberofitems > 1) {
        ...
        for (var i = 0; i < numberofitems; i++) {
          ...
          items.push(
            "<div class='w3-third' id='" +
              id +
              "'><div class='w3-container w3-margin w3-panel w3-card-2 '><div class='w3-bar'><h3 class='w3-bar-item'><a target='_blank' href='/" +
              coll +
              "/" +
              id +
              "/" +
              ...
              "'>" +
              title +
              "</a></h3> <span class='w3-badge w3-bar-item w3-right'>" +
              match.hitsCount +
              "</span></div><div lang='gez' class='word'>" +
              parsedtext +
              "</div></div></div>",
          );
        }
        ...
        if (numberofitems == 1) {
          ...
          items.push(
            '<div  class="w3-third w3-panel w3-card-4 w3-padding-64"><h3><a href="' +
              url +
              '">' +
              title +
              "</a><span class='w3-badge'>" +
              match.hitsCount +
              "</span></h3><div lang='gez' class='word'>" +
              parsedtext +
              "</div></div>",
          );
  ...
  $('div#attestations [lang="gez"] p span').each(function (wn) {
    ...
    $.each(words, function (i, v) {
      ...
      if (i == countwords - 1) {
        $(word).append(
          $(
            "<span class='alpheios-word attpopup' data-value='atp" +
              wn +
              i +
              "'>" +
              nostops.w +
              nostops.stop +
              "\
                <span class='popuptext w3-hide w3-tiny w3-padding' id='atp" +
              wn +
              i +
              "'>\
                Search " +
              nostops.w +
              " :<br/>\
                <a href='/newSearch.html?searchType=text&mode=none&query=" +
              nostops.w +
        ...
        $(word).append(
          $(
            "<span class='alpheios-word attpopup' data-value='atp" +
              ...
              nostops.w +
              "&searchType=text&mode=none' target='_blank'>in Beta maṣāḥǝft</a><br/>\
                <a href='/morpho?query=" +
              nostops.w +
              "' target='_blank'>in the Gǝʿǝz Morphological Parser</a><br/>\
                <a href='/morpho/corpus?query=" +
