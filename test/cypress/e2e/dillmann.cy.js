describe("Dillman page", () => {
  ...
  it("should display Beta version warning", () => {
    cy.get("#body").should("contain", "This app is a prototype Beta version.");
  });
  ...
  describe("lemma search with mouse", () => {
    ...
    it("should show results", () => {
      ...
      // 03_user 18.4
      cy.get("#results > .w3-row").should("be.visible");
      cy.get("h3").should("contain", 'You found "ሀሰሰ" in ');
      cy.get("#results")
        .invoke("attr", "data-template-per-page")
        .then((value) => {
          const pagination_int = parseInt(value);
          cy.get("#results > .w3-row").its("length").should("be.lte", pagination_int);
        });

      cy.get("#results .w3-twothird > a")
        .first()
        .invoke("attr", "href")
