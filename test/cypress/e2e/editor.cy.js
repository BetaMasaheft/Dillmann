...
describe("Single Lemma", () => {
  ...
  describe("Edit existing entry", () => {
    it("opens the update form pre-filled, without saving", () => {
      ...
      cy.get(".w3-panel").invoke("text").should("include", "You are updating");

      cy.get("#updateEntry").should("exist");
      cy.get('input[name="form"]').should("exist").invoke("val").should("not.be.empty");
    ...
    itWrites("submits the edit via Confirm and saves successfully (writes a new version)", () => {
      ...
      cy.get("#msg").clear().type("cypress write test — disposable stack only");
      ...
      cy.get("body")
        ...
        .then((text) => {
          ...
          expect(text, "upconversion succeeded (#535)").to.not.match(
            /Unable to set up transformer|stylesheet compilation|upconvertSense/,
          );
