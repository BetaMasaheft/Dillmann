...
for $annotation in subsequence($data, 1, 1) 
let $annoID := 'traces:' ||string($annotation/@xml:id)
let $morph := for $m in $annotation//t:fs[@type="morpho"]/t:f[not(@name='lex')] return string($m/@name) || ' ' || $m/text()
let $tokens := if($annotation//t:fs[@type='tokens']) then 
(
for $token in $annotation//t:fs[@type='tokens']/t:f[@name='lit']
let $tokenURI := string($annotation/@xml:id) || '_'||string($token/@fVal)
let $morphtok := for $m in $token//t:fs[@type="morpho"]/t:f[not(@name='lex')] return string($m/@name) || ' ' || $m/text()
return
...
let $query := 'PREFIX traces: <https://betamasaheft.eu/morpho/>
...
return 
$query
(:fusekisparql:update('traces', $operation, $annotation):)