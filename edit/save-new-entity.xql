  ...
  <html>
    ...
    <body>
      <div id="confirmation">
        ...
        <a class="btn btn-success" href="/Dillmann/newentry.html">back to list</a>
      </div>
    </body>
  </html>
) else
...
type="application/xml" schematypens="http://purl.oclc.org/dsdl/schematron"'
    ...
    <TEI xmlns="http://www.tei-c.org/ns/1.0" xml:lang="en">
      <teiHeader>
        <fileDesc>
          <titleStmt><title xml:lang="{ $formlang }">{ $form }</title><editor>Alessandro Bausi</editor></titleStmt>
          <publicationStmt>
          ...
          <sourceDesc>
            ...
            <p>Additional records were created by the project <ref
                ...
                xml:id="traces"
              >
                  TraCES (ERC Advanced Grant, Grant Agreement 338756) and follow-up projects at HLCEES</ref>
            </p>
            <p>Sources include also <ref
                target="https://archive.org/details/supplmentaulexic0000grba"
                xml:id="grebaut"
              >Sylvain Grébaut, Supplément au lexicon linguae aethiopicae de August Dillmann: (1865) et édition du lexique de Juste d'Urbin (1850 - 1855), Paris: Imprimerie Nationale, 1952</ref> and <ref
                xml:id="leslau"
              >Wolf Leslau, Comparative Dictionary of Geʻez (Classical Ethiopic): Geʻez-English, English-Geʻez, with an Index of the Semitic Roots, Wiesbaden: Harrassowitz, 1987</ref>
            </p>
          </sourceDesc>
        </fileDesc>
        <encodingDesc>
        ...
        <revisionDesc>
          <change when="{ format-date(current-date(), "[Y0001]-[M01]-[D01]") }" who="{ $editor }">{ $msg }</change>
        </revisionDesc>
      </teiHeader>
      <text>
        <body>
          <div>
            <entry n="{ $nexN }" xml:id="{ $newid }">
              <form><foreign xml:lang="{ $formlang }">{ $form }</foreign></form>
              {
                for $s in $eachsense//t:sense
                return $s
              }
            </entry>
          </div>
  ...
  return if ($validation = true()) then (
    ...
    let $addxmlids :=
      for $sensewithoutid in $record//t:sense[@n]
      let $mainSense := $sensewithoutid/ancestor::t:sense[@source]
      let $parentSense :=
        for $pS in $sensewithoutid/ancestor::t:sense[@n]
...
let $log := log:add-log-message('/Dillmann/lemma/'||$newid,sm:id()//sm:real/sm:username/string(), 'created')
 :)(: confirmation page with instructions for editors :)
    return <html>
      ...
      <body>
        <div class="col-md-4 col-md-offset-4 alert alert-success" id="confirmation">
          <h1 class="lead">Thank you very much { sm:id()//sm:real/sm:username/string() }!</h1>
          <p> Your entry for 
                        <a
              href="/Dillmann/lemma/{ substring-before($file, ".xml") }"
              target="_blank"
            ><span class="lead">{ $form }</span></a> has been saved!</p>
          {
            if ($editorsnotification = "yes") then (
          ...
          <a href="/Dillmann/newentry.html">Create another entry!</a>
      ...
      <body>
        <div class="col-md-12 alert alert-warning">
          <div class="col-md-6">
            <p
              class="lead"
            >Sorry, the document you are trying to save is not valid. There is probably an error in the content somewhere. Below you can see the report from the schema. Beside the XML, check it out or send the link or a screenshoot to somebody for help.</p>
