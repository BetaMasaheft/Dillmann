...
let $updateform := update replace doc($targetfileuri)//tei:entry/tei:form with <form
  xmlns="http://www.tei-c.org/ns/1.0"
>{ $transForm }</form>

return <html>
  <head><title>{ $title }</title></head>
  <body><h1>{ $title }</h1><p>Item { $id } has been updated.</p><a href="/Dillmann/lemma/{ $id }">{ $form }</a></body>
</html>
