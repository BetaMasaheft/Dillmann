...
declare function local:mergeMain($a, $b as item()*) as item()* {
  ...
  ) else if (empty($b) or $a[1] lt $b[1]) then (
    <div class="left" style="width: 50%; float: left;">{ $a[1]//text() }</div>, local:mergeMain(subsequence($a, 2), $b)
  ) else if (empty($a) or $a[1] gt $b[1]) then (
    <div class="right" style="width: 50%; margin-left:50%;">{ $b[1]//text() }</div>,
    local:mergeMain($a, subsequence($b, 2))
  ) else (
    <div class="match" style="width: 100%; text-align: center;">{ $a[1]//text() }</div>,
    local:mergeMain(subsequence($a, 2), subsequence($b, 2))
  )
};
...
declare function local:mergeDeep($a, $b as item()*) as item()* {
  if (empty($a) and empty($b)) then (
    ...
    <span style="color: red;">{ $a[1] }</span>, local:mergeDeep(subsequence($a, 2), $b)
  ) else if (empty($a) or $a[1] gt $b[1]) then (
    <span style="color: green;">{ $b[1] }</span>, local:mergeDeep($a, subsequence($b, 2))
  ) else (
    <span>{ $a[1] }</span>, local:mergeDeep(subsequence($a, 2), subsequence($b, 2))
  )
};

let $parametersName := request:get-parameter-names()
(: Check authentication - user must be logged in and in lexicon or dba group :)(: Note: login:set-user in controller should establish session, but we check here to be safe :)
let $currentUser := sm:id()//sm:real/sm:username/string()
let $userGroups := if ($currentUser != "guest") then
...
return if (not($isAuthenticated)) then
  ...
  } catch * { util:log("warn", "Could not store backup: " || $err:description) }

  let $log := if ($store) then
    log:add-log-message($backupfilename, sm:id()//sm:real/sm:username/string(), "backup")
  else (
  )
  return if (contains($parametersName, "sense")) then (
    ...
    let $temporary := (: let $form := $record//tei:form//tei:foreign//text()
return :) <TEI xmlns="http://www.tei-c.org/ns/1.0" xml:lang="la">
      <teiHeader>
        <fileDesc>
          <titleStmt><title xml:lang="{ $formlang }">{ $form }</title><author>Alessandro Bausi</author></titleStmt>
          <publicationStmt>
            ...
            <availability>
              <licence
                target="https://creativecommons.org/licenses/by-sa-nc/4.0/"
              >
          ...
          <sourceDesc>
            <p>A thoroughly elaborated txt version of <ref
            ...
            <p>
              <ref
                ...
                xml:id="traces"
              >ERC Advanced Grant
                  TraCES (Grant Agreement 338756) and follow-up projects</ref>
            </p>
          </sourceDesc>
        </fileDesc>
        ...
        <body>
          <div>
            <entry>
              ...
              {
                for $s in $eachsense//tei:sense[@xml:lang][@source]
                return $s
              }
            </entry>
          </div>
    ...
    return if ($validation = true()) then (
      ...
      let $sensesLang :=
        ...
        return $lang
      let $eachLang :=
        for $lang in $eachsense//tei:sense[@source]/@xml:lang
        return $lang
      ...
      let $updateform :=
        ...
        return if ($sensesArray[@xml:lang = $slang]) then (
          ...
          update insert $s into $item//tei:entry
        )
      ...
      let $addxmlids :=
        for $sensewithoutid in $sensesArray//tei:sense[@n]
        let $mainSense := $sensewithoutid/ancestor::tei:sense[@source]
        let $parentSense :=
          for $pS in $sensewithoutid/ancestor::tei:sense[@n]
        ...
        return update insert attribute xml:id { $newId } into $sensewithoutid
      ...
      let $addroot := if ($root = "root" and $record//tei:form[not(descendant::tei:rs[@type = "root"])]) then
        update insert $segRoot into doc($targetfileuri)//tei:form
      else (
      )
      let $updatemainForm := update replace $record//tei:form//tei:foreign//text() with $form

      let $updateFuseki := try { updatefuseki:entry($record, "INSERT") } catch * {
        util:log("warn", "failed to update fuseki")
      }
      ...
      let $backupedfile := if ($store and doc-available(concat($backup-collection, "/", $backupfilename))) then
        doc(concat($backup-collection, "/", $backupfilename))
      else
        $item
      ...
      return <html>
        ...
        <body>
          <div class="alert alert-success">
            <h2>{ $title }</h2>
            <p class="lead">Dear <span id="user">{ $cU }</span>, Lemma  <a href="/Dillmann/lemma/{ $id }" id="filename">
                { $filename }
              </a> has been updated successfully!</p>
            <p id="msg">{ $cU } left this message after editing: { $msg }</p>
            {
              if ($notifyEditors = "yes") then
                <p id="notifyEditors">A notification email has been sent to the editors.</p>
              else
                <p
                >You have not notified the editors about this change. If you wish to do so, please tick the corresponding box next time.</p>
            }
          ...
          <div class="alert alert-info">
            ...
            <div id="old" style="display: none;">
              { transform:transform($backupedfile, "xmldb:exist:///db/apps/gez-en/xslt/txt.xsl", ()) }
            </div>
